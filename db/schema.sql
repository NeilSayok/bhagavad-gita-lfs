...
-- Image paths carry a {size} placeholder the client substitutes.
--
-- schema v2: every field the source JSON now carries per-language
-- (chapter name/translation/transliteration, verse transliteration,
-- word-meaning transliteration) lives in its *_translation / *_text table
-- rather than as a single column on the parent row.
...
CREATE TABLE chapter (
    ...
    img_landscape   TEXT NOT NULL,               -- 'chapters/{size}/chapter_1/landscape/img.png'
...
CREATE TABLE chapter_translation (
    ...
    name            TEXT,                        -- chapter.name, e.g. 'अर्जुनविषादयोग'
    translation     TEXT,                        -- chapter.translation, e.g. 'Arjuna Visada Yoga'
    transliteration TEXT,                        -- chapter.transliteration, e.g. 'Arjun Viṣhād Yog'
    meaning         TEXT,
    summary         TEXT,
...
CREATE TABLE verse (
    ...
    img_landscape   TEXT NOT NULL,               -- 'sloks/{size}/chapter_1/slok_1/landscape/img.png'
    img_portrait    TEXT NOT NULL,
    img_square      TEXT NOT NULL
);
...
CREATE TABLE verse_text (
    ...
    speaker         TEXT,
    slok            TEXT,
    transliteration TEXT,                        -- top-level slok.transliteration, per script
    PRIMARY KEY (verse_id, lang_code)
);
CREATE INDEX idx_verse_text_lang ON verse_text(lang_code);

-- Meaning-level per-verse prose.
...
CREATE TABLE word_meaning (
    ...
    sanskrit        TEXT NOT NULL,              -- only non-translated field in the block
